...
export function updateAlbumServerCache(albumPath: string): void {
    ...
    fetch(Config.refreshAlbumCacheUrl(albumPath), requestConfig)
        .then((response) => {
            if (response.status !== 200) throw new Error('Error fetching: ' + response.statusText);
            return response;
        })
        .then((response) => response.json())
        .then((json) => {
            if (json.status !== 'success') throw new Error('Error refreshing cache for album ${albumPath}');
        })
        .then(() => {
