...
SET VARIABLE cf_files = source_files(['../../dumps/cloudfront/**/*.gz', '../../dumps/cloudfront/**/*.tsv']);

-- One row per line of every file, header lines included, each carrying what its
-- file's header declared. The unit separator never occurs in a log line, so each
-- line lands whole in one column; quote and escape are off so a stray quote in a
-- user agent cannot swallow the rest of the file.
CREATE TEMP TABLE cf_lines AS
SELECT
  filename,
  line,
  string_split(line, E'\t') AS cols,
  count(*) FILTER (line LIKE '#Fields:%') OVER file AS header_lines,
  any_value(string_split(regexp_replace(line, '^#Fields: ', ''), ' ')) FILTER (line LIKE '#Fields:%') OVER file AS fields
FROM read_csv(getvariable('cf_files'),
  columns = {'line': 'VARCHAR'}, delim = E'\x1F', quote = '', escape = '',
  header = false, comment = '', filename = true)
WINDOW file AS (PARTITION BY filename);
...
SELECT
  regexp_replace(filename, '^.*/dumps/cloudfront/', '') AS source_file,
  regexp_extract(filename, 'dumps/cloudfront/([^/]+)/([^/]+)/', 1) AS env,
  regexp_extract(filename, 'dumps/cloudfront/([^/]+)/([^/]+)/', 2) AS distribution,
  any_value(header_lines) AS header_lines,
  any_value(fields) AS fields,
  count(*) FILTER (line NOT LIKE '#%') AS rows,
  count(*) FILTER (line NOT LIKE '#%' AND len(cols) <> len(fields)) AS malformed_rows,
  filename
FROM cf_lines
GROUP BY ALL;
...
SELECT
  ...
  -- With the IP and edge, identifies the viewer's connection.
  ...
  result_type IN ('Hit', 'RefreshHit') AS is_cache_hit,
  ...
  source_file
FROM cf_lines
JOIN (SELECT filename, env, distribution, source_file FROM cloudfront_files) USING (filename)
WHERE line NOT LIKE '#%' AND header_lines = 1 AND len(cols) = len(fields);
...
SELECT 'missing_required_field',
       source_file || ' lacks ' || field
FROM cloudfront_files, unnest([
  ...
  'ssl-protocol', 'sc-content-type', 'x-edge-request-id']) AS required(field)
WHERE header_lines = 1 AND NOT list_contains(fields, field)
