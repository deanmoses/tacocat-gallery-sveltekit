...
-- sometimes just a sentence. Every function in the stack logs to one group, so
-- the function is read from the stream name,
...
SELECT
  ...
  nullif(regexp_extract(logStreamName, '^\d{4}/\d{2}/\d{2}/tacocat-gallery-sam-[a-z]+-([A-Za-z0-9]+)\[', 1), '') AS function_name,
  nullif(regexp_extract(logStreamName, '\]([0-9a-f]+)$', 1), '') AS instance,
  try_cast(message AS JSON) AS record,
  -- The object a handler logged, whichever of its two shapes it arrived in;
  -- NULL for a sentence.
  try_cast(record ->> '$.message' AS JSON) AS logged,
...
COMMENT ON TABLE lambda_requests IS 'GRAIN: one row per request a handler logged on arrival, by Lambda request id. For a derived image, `path` is what CloudFront asked for after its rewrite: /i/<media path>/<version>/<size>, then /crop=<x,y,w,h> for a cropped one, and `cf_request_id` is the CloudFront row''s request_id; the API handlers have no CloudFront in front of them and log none. `has_token` is whether an API request carried an id_token cookie, valid or not; NULL for the resizer, which has no cookies.';
...
SELECT
  ...
  -- Present only when the invocation had to start an execution environment first.
  try_cast(e.record ->> '$.record.metrics.initDurationMs' AS DOUBLE) AS init_ms,
  init_ms IS NOT NULL AS is_cold,
  ...
  -- The report is written when the invocation ends.
  e.ts - INTERVAL (coalesce(init_ms, 0) + coalesce(duration_ms, 0)) MILLISECOND AS started_ts,
  epoch(started_ts - lag(e.ts) OVER (PARTITION BY e.env, e.function_name ORDER BY e.ts)) AS idle_seconds,
  EXISTS (
    SELECT 1 FROM gateway_requests g
    WHERE g.env = e.env AND g.lambda_request_id = e.request_id AND g.is_probe
  ) AS is_probe,
